<?php
	...
	if(@$id_kamar && @$_GET['req']=='editkmr'){
		$qry = $konek->query("SELECT kamar.*, harga.*, kos.id_kos, kos.id_akun FROM kamar, harga, kos WHERE kamar.id_kamar=harga.id_kamar AND kamar.id_kamar='$id_kamar' AND kos.id_kos=kamar.id_kos AND kos.id_akun='".$_SESSION['id']."'");
	...
	<form action="" method="post" enctype="multipart/form-data">
		<table class="profile-table">
			...
			<td width="120px">Nama Kos</td>
				...
				<td>
					<select name="nama_kos">
						<option value=""></option>
						<?php
						$q=$konek->query("SELECT id_kos, nama, id_akun FROM kos WHERE id_akun='".$_SESSION['id']."'");
						while($r=$q->fetch_object()){
			...
			<tr>
				...
				<td>
					<?php
					$check = isset($_POST['fasilitas']) ? $_POST['fasilitas'] : array();
